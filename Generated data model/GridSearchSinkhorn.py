...
def train_and_validate(config, train_loader, val_loader, device, epochs=20):
    ...
    encoder = SinkhornSetNet(input_dim=input_dim, hidden_dim=hidden_dim, emb_dim=emb_dim,
                             cost_type=cost_type, sinkhorn_reg=sinkhorn_reg, 
                             sinkhorn_iters=sinkhorn_iters).to(device)
    ...
    for epoch in range(epochs):
        ...
        for A_batch, B_batch, y_batch, mask_batch in train_loader:
            ...
            # Forward pass: returns transport cost
            pred_costs = encoder(A_batch, B_batch, mask=mask_batch)
            
            # Convert similarity label to cost label
            y_cost = 1.0 - y_batch
...
def main():
    ...
    param_grid = {
        'hidden_dim': [64, 128],
        'sinkhorn_iters': [10, 20, 30],
        'sinkhorn_reg': [0.1, 0.05],
        'lr': [1e-3, 1e-4, 1e-2],
    }
    ...
    print(f"Starting Sinkhorn Grid Search with {len(combinations)} configurations...")
    ...
    for i, config in enumerate(combinations):
        ...
        val_loss, _ = train_and_validate(full_config, train_loader, val_loader, device, epochs=full_config['epochs'])
    ...
    final_loss, best_encoder = train_and_validate(best_config, train_loader, val_loader, device, epochs=best_config['epochs'])
    ...
    with torch.no_grad():
        for A_batch, B_batch, y_batch, mask_batch in test_loader:
            ...
            # Predict cost
            pred_cost = best_encoder(A_batch, B_batch, mask=mask_batch)
            
            # Ground truth cost
            y_cost = 1.0 - y_batch
            
            y_true_all.append(y_cost.cpu().numpy())
            y_pred_all.append(pred_cost.cpu().numpy())
